...
def _synthetic_sessions(session_count: int, candles_per_session: int) -> tuple[pd.DataFrame, ...]:
    ...
    for session_index in range(session_count):
        ...
        timestamp = pd.Timestamp(session_date, tz="UTC") + pd.Timedelta(hours=14, minutes=30)
        features = generator.normal(size=(candles_per_session, len(FEATURE_COLUMNS))).astype(
            np.float32
        )
        ...
        sessions.append(pd.DataFrame({
            ...
            "ts_event": pd.date_range(timestamp, periods=candles_per_session, freq="5min"),
            **{
