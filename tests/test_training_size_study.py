...
def make_sessions():
    sessions = []
    for index, timestamp in enumerate(pd.date_range('2016-01-01', periods=176, tz='UTC')):
        sessions.append(pd.DataFrame({
            'session_date': [timestamp.date()], 'ts_event': [timestamp],
            'candlestick_id': [str(timestamp)], 'open_gap': [index / 1000],
            'body': [0.01], 'high_from_close': [0.02], 'low_from_close': [-0.01],
            'current_target': [index % 3], 'anticipated_target': [(index + 1) % 3],
        }))
...
def test_all_prefixes_share_the_exact_forward_holdout():
    ...
    for count in study.TRAINING_COUNTS:
        ...
        assert all(actual is expected for actual, expected in zip(training, sessions[:count]))
        assert len(training) == count
        assert len(holdout) == 26
        assert all(actual is expected for actual, expected in zip(holdout, sessions[150:]))
...
def test_fixed_training_is_fresh_train_only_and_reusable(tmp_path, monkeypatch):
    sessions = make_sessions()
    cache = {id(session): tuning.convert_session_to_tensors(session, timestamp_column='ts_event')
             for session in sessions}
    settings = replace(tuning.TuningSettings(), model_dimension=12, epochs=1, context_length=4)
    ...
    for count in study.TRAINING_COUNTS:
        training, holdout = study.select_sessions(sessions, count)
        reports.append(study.run_fixed_candidate(
            training, holdout, cache, settings, tmp_path, 'frozen',
        ))
        saved = torch.load(tmp_path / f'train_{count:03d}' / 'checkpoint.pt', weights_only=True)
        torch.testing.assert_close(saved['feature_mean'], original_fit(training).mean)
    ...
    assert all(report['holdout_dates'] == reports[0]['holdout_dates'] for report in reports)
    ...
    previous_bytes = (tmp_path / 'train_100' / 'checkpoint.pt').read_bytes()
    reused = study.run_fixed_candidate(training, holdout, cache, settings, tmp_path, 'frozen')
    assert reused['model_state_sha256'] == reports[0]['model_state_sha256']
    assert (tmp_path / 'train_100' / 'checkpoint.pt').read_bytes() == previous_bytes
    with pytest.raises(ValueError, match='identity changed'):
        study.run_fixed_candidate(
            training, holdout, cache, replace(settings, epochs=2), tmp_path, 'frozen',
        )
    report_path = tmp_path / 'train_100' / 'report.json'
    report_path.write_text(report_path.read_text() + ' ')
    with pytest.raises(ValueError, match='identity changed'):
        study.run_fixed_candidate(training, holdout, cache, settings, tmp_path, 'frozen')
...
def test_final_epoch_is_kept_even_when_reported_training_loss_increases(tmp_path, monkeypatch):
    ...
    study.run_fixed_candidate(training, holdout, cache, settings, tmp_path, 'frozen')
    weights = torch.load(tmp_path / 'train_100' / 'checkpoint.pt', weights_only=True)
    assert all(torch.equal(value, trained_states[-1][name])
               for name, value in weights['model_state'].items())
    assert any(not torch.equal(value, trained_states[0][name])
               for name, value in weights['model_state'].items())
