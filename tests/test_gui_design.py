...
def test_regime_buttons_save_pair_and_update_progress(qt_application,candlestick_data,tmp_path):
    window=AnnotationWindow(candlestick_data,tmp_path/'annotations.db')
    try:
        assert window.property('pricesanityTheme')=='shared-v1'
        assert all(button.minimumHeight()>=40 for button in window.findChildren(QPushButton))
...
def test_annotation_fits_scaled_1080p(qt_application,candlestick_data,tmp_path):
    from pricesanity.gui.theme import apply_theme
    from PySide6.QtCore import QRect
    from unittest.mock import patch
    window=AnnotationWindow(candlestick_data,tmp_path/'scaled.db')
    class SmallScreen:
        def availableGeometry(self):return QRect(0,0,1280,720)
    try:
        with patch.object(AnnotationWindow,'screen',return_value=SmallScreen()):
            apply_theme(window)
        window.resize(1280,700)
        window.show()
        qt_application.processEvents()
        assert window.height()<=700
        assert all(button.minimumHeight()>=44 for button in window.findChildren(QPushButton))
    finally:window.close()


def test_scores_get_separate_labeled_axes(qt_application):
    frame=pd.DataFrame({'session_date':['2026-01-05']*3,'uncertainty_kind':['probability_estimate']*3})
    for kind in ('human','predicted'):
        for head in ('current','anticipated'):frame[f'{kind}_{head}_regime']=['bull','range','bear']
    for head in ('current','anticipated'):
        for regime in ('bull','bear','range'):frame[f'{head}_probability_{regime}']=[.3,.3,.4]
    scores=frame.copy();scores['uncertainty_kind']='uncalibrated_decision_score'
    for head in ('current','anticipated'):
        for regime in ('bull','bear','range'):scores[f'{head}_score_{regime}']=[-1.,2.,.5]
    canvas=_ABSessionCanvas()
    try:
        canvas.show_session(frame,scores,first_name='A',second_name='B',ohlc=None)
        ...
        assert canvas.figure.axes[2].get_ylim()==(0.,1.)
    finally:canvas.close()


def test_populated_leaderboard_keeps_cells_in_their_rows(qt_application,monkeypatch,tmp_path):
    ...
    metrics=evaluate_benchmark_predictions(human_current=np.array([0,1]),predicted_current=np.array([0,0]),human_anticipated=np.array([0,2]),predicted_anticipated=np.array([0,0]),session_indices=np.array([0,0])).to_dict()
    runs=[]
    for index,name in enumerate(('logistic_regression','gaussian_naive_bayes')):
        identity={**_identity().to_dict(),'model_name':name,'run_name':'final_seed_42','model_configuration_sha256':str(index)}
        runs.append(ComparisonRun(name,tmp_path/name,{'identity':identity},metrics))
    def forbidden(*args,**kwargs):raise AssertionError('Leaderboard read predictions eagerly')
    monkeypatch.setattr(comparison,'load_aligned_comparison_predictions',forbidden)
    window=ModelComparisonWindow(tuple(runs))
    try:
        assert window.leaderboard.rowCount()==2
        for row in range(2):
            assert all(window.leaderboard.item(row,col) is not None for col in range(15))
        assert {window.leaderboard.item(row,1).text() for row in range(2)}=={'logistic_regression','gaussian_naive_bayes'}
    finally:window.close()
